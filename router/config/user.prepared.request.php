...
function reinit_password_user($token_passwd, $email)
{
	...
	$SQL_QUERY = $conn->prepare("UPDATE camagru.users SET passwd=:passwd WHERE email=:email");
...
function email_user($login)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare("SELECT email FROM camagru.users WHERE login=?");
...
function login_get_user($subscribe_email, $login, $passwd)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare("SELECT * FROM camagru.users WHERE subscribe_email=? AND login=? AND passwd=?");
...
function add_user($login, $email, $passwd, $token)
{
	...
	$SQL_QUERY 	= $conn->prepare('
			INSERT INTO
			camagru.users (id, email, login, passwd, subscribe_email, token_subscribe, pic_reference, pic_liked, notification_like, notification_comment) 
...
function user_is_subscribe_email($value)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare("SELECT * FROM camagru.users WHERE email=?");
...
function user_is_subscribe_login($value)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare("SELECT * FROM camagru.users WHERE login=?");
	$SQL_QUERY->execute([$value]);
	$result = $SQL_QUERY->fetchAll();
	return ($result);
}
...
function update_database_likes($new_value, $old_value)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare("UPDATE camagru.likes SET login=:login WHERE login=:oldLogin");
...
function update_database_comments($new_value, $old_value)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare("UPDATE camagru.comments SET login=:login WHERE login=:oldLogin");
	$SQL_QUERY->bindValue(':login', $new_value, PDO::PARAM_STR);
	$SQL_QUERY->bindValue(':oldLogin', $old_value, PDO::PARAM_STR);
	$SQL_QUERY->execute();
}
...
function update_database_pictures($new_value, $old_value)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare("UPDATE camagru.pictures SET auteur=:login WHERE auteur=:oldLogin");
	$SQL_QUERY->bindValue(':login', $new_value, PDO::PARAM_STR);
	$SQL_QUERY->bindValue(':oldLogin', $old_value, PDO::PARAM_STR);
	$SQL_QUERY->execute();
}
...
function update_user($login, $email, $passwd, $old_email)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare('UPDATE camagru.users SET login=:login, email=:email, passwd=:passwd WHERE email=:oldEmail');
	$SQL_QUERY->bindValue(':login', $login, PDO::PARAM_STR);
	$SQL_QUERY->bindValue(':email', $email, PDO::PARAM_STR);
	$SQL_QUERY->bindValue(':passwd', $passwd, PDO::PARAM_STR);
...
function remove_like($login, $path)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare('DELETE FROM camagru.likes WHERE login=? AND picture_path=?');
...
function add_like ($login, $path)
{
	...
	$conn 		= new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY 	= $conn->prepare('INSERT INTO camagru.likes (id, login, picture_path) VALUES (:id, :login, :pathPicture)');
...
function is_like($login, $path)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare("SELECT * FROM camagru.likes WHERE login=? AND picture_path=?");
	$SQL_QUERY->execute([$login, $path]);
	$result = $SQL_QUERY->fetchAll();
	return ($result);
}
...
function increase_like($picture_path)
{
	...
	$conn = new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY = $conn->prepare('UPDATE camagru.pictures SET nbr_like = nbr_like + 1 WHERE picture_path=?');
...
function add_comment($login, $text, $path, $date)
{
	...
	$conn 		= new PDO($DB_DSN, $DB_USER, $DB_PASS);
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	$SQL_QUERY 	= $conn->prepare('INSERT INTO camagru.comments (id, login, comment, picture_path, posted_date) VALUE (0, :login, :textComment, :pathPicture, :dateComment)');
	$SQL_QUERY->bindValue(':login', $login, PDO::PARAM_STR);
